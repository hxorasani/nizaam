...
hooks.set('timeline.restart', function () { direction = !direction; });
...
//		loc.y = ease[ ease.all[chosen_ease] ](timeline.now, start , end , timeline.length);
//		rot.y = ease[ ease.all[chosen_ease] ](timeline.now, start2, end2, timeline.length);
//		rot.z = ease[ ease.all[chosen_ease] ](timeline.now, start2, end2, timeline.length);
		o.update();
	});

	var rot = balls[0].get('rotation');
	var source = balls[0].get('location');
	var target = balls[1].get('location');
	...
	var str = source.x+' '+source.y+' '+source.z+' source\n'
			+ target.x+' '+target.y+' '+target.z+' target\n'
			+ parsefloat(timeline.now/1000, 2)+'s - '+timeline.frame+'f - '+get_fps()+'fps\n'
...
function on_keyboard (m) {
	...
	if (!yes) yes = timeline.keyboard(m);
	return yes;
}
