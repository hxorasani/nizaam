#ifndef SMALL3DLIB_H
#define SMALL3DLIB_H
/* @hxorasani changes i've made
 * 2023-03-10 removed const from verts, tris, indices... for eff mem man
 * */
#include <stdint.h>
...
#define S3L_CUBE_TEXCOORDS(m)\
  ...
  m,0,  0,m,  m,m,\
  0,0,  0,m,  m,0
#define S3L_UNUSED(what) (void)(what)
...
#endif
static inline void S3L_PIXEL_FUNCTION(S3L_PixelInfo *pixel);
typedef struct
{
...
uint8_t _S3L_projectedTriangleState = 0;
  ...
  S3L_Index triangleIndex)
{
  ...
  S3L_Vec4 *tPointSS, *lPointSS, *rPointSS; 
  ...
  *barycentric2 = S3L_F - 2 * (S3L_F / 3);
  ...
  S3L_ScreenCoord splitY;
  S3L_ScreenCoord endY;
  int splitOnLeft;        
  ...
  #endif
  S3L_Unit
    tPointRecipZ, lPointRecipZ, rPointRecipZ, 
  ...
  while (currentY < endY)   
  {
    ...
    { 
      p.y = currentY;
#if !S3L_FLAT
      S3L_Unit rowLength = S3L_nonZero(rX - lX - 1);
  #if S3L_PERSPECTIVE_CORRECTION
...
#if S3L_PERSPECTIVE_CORRECTION
      S3L_ScreenCoord i = lXClipped - lX;  
#endif
